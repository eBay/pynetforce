...
check_identical_policy_files () {
    ...
    # independent of whether it's executed from the netforce source tree or from
    # site-packages installation path. We don't want two copies of the same
    # file to diverge, so checking that they are identical
    diff etc/policy.json netforce/tests/etc/policy.json 2>&1 > /dev/null
    if [ "$?" -ne 0 ]; then
